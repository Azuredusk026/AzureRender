...
The active application is in [`Project/AzureRender`](Project/AzureRender). The current engineering baseline is S36.2, while the active long-term milestone is M1 Character Rendering Quality Foundation. The deferred Multi-pass/Subpass/DRLR benchmark begins only after the portfolio release gate.
