...
最终可见性排查确认 Brow primitive、Face D、UV、透明排序和 WPO 均已生效。资产审计进一步确认该 primitive 是 34 个眉毛/睫毛拓扑小岛的合集，分别受 `eye*`、`eyelash*` 和 `brow*` 骨骼影响；所有 578 个顶点的权重和均为 1，左右分配对称，没有无关骨骼或退化三角形。禁止围绕单一中心缩放整块几何，否则会把不同骨骼控制的小岛拉开。当前 `styleParameters.x=0.01 m` 只负责局部上移，`styleParameters.y=2.0` 在片元阶段沿 Face D 纹理纵向扩展深红笔画两个 texel，增加远景宽度但不改变顶点、权重或睫毛位置。验收必须同时检查近景、全身和完整转台。

## 最终主光与阴影基线（2026-08-19）

旧 Endfield Beauty 的 HDRI 漫反射占比过高、填充光为 0.30，主光又接近顶光；转台八秒只旋转约 92°，因此材质数值虽变化，观看时仍接近均匀照明。当前主光改为固定世界空间侧上方方向 `(0.62, 0.68, 0.38)`，环境漫反射在 Endfield Look 中降为原来的 58%，填充降为 0.10，主光提高为 1.38，并加强实时 shadow-map 与 Lam 阴影染色。角色 Portfolio 从画面朝左的 `3π/2` 起始角开始，转速固定为 `2π/16 rad/s`，每个 16 秒展示段精确完成一圈并回到朝左；黑洞 Portfolio 起始角和速度保持不变。
