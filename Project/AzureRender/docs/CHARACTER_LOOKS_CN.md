...
最终可见性排查确认 Brow primitive、Face D、UV、透明排序和 WPO 均已生效；真正问题是骨骼 glTF 导出的卡片落在上眼睑边缘，同时原始 Face D 红色经过 HDR 合成后与肤色对比不足。`brow-overlay` 使用 `styleParameters.x=0.01 m` 做数据化局部上移，并在保留 Face D 纹理细节的前提下压低绿色/蓝色响应。为保证全身正面构图仍有稳定像素覆盖，`styleParameters.y=2.0` 以导出卡片中心 `styleParameters.z=1.2536046 m` 为轴放大纵向几何；这是真实卡片加宽，不是单纯加深颜色。验收必须同时检查 `face-front` 与全身正面帧。

## 最终主光与阴影基线（2026-08-19）

旧 Endfield Beauty 的 HDRI 漫反射占比过高、填充光为 0.30，主光又接近顶光；转台八秒只旋转约 92°，因此材质数值虽变化，观看时仍接近均匀照明。当前主光改为固定世界空间侧上方方向 `(0.62, 0.68, 0.38)`，环境漫反射在 Endfield Look 中降为原来的 58%，填充降为 0.10，主光提高为 1.38，并加强实时 shadow-map 与 Lam 阴影染色。角色 Portfolio 转速固定为 `2π/16 rad/s`，每个 16 秒展示段精确完成一圈；黑洞 Portfolio 速度保持不变。
