...
| 顺序 | 任务 | 状态 | 目标 | 依赖 | 预定 Commit 标题 |
|---:|---|---|---|---|---|
| 1 | AR-5.0 | Ready | 可重复的本地/CI 发布门禁编排与结果摘要 | AR-4.5 | `完成 AR-5.0 发布门禁编排` |
| 2 | AR-5.1 | Backlog | 类型化 CLI 解析、稳定错误类型与参数契约测试 | AR-5.0 | `完成 AR-5.1 命令行契约` |
| 3 | AR-5.2 | Backlog | `.azscene` 原子保存、临时文件清理和恢复测试 | AR-5.1 | `完成 AR-5.2 场景原子保存` |
