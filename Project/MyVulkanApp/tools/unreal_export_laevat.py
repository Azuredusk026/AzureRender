...
OUTPUT_PATH = os.path.normpath(
    ...
    r"\assets_private\laevat_static\laevat_static.glb"
)
...
def main():
    ...
    unreal.log(f"Export source: {asset.get_path_name()}")
    unreal.log(f"Export class: {asset.get_class().get_name()}")

    options = unreal.GLTFExportOptions()
    options.export_uniform_scale = 0.01
    options.export_vertex_colors = False
    options.export_vertex_skin_weights = False
    ...
    if not success:
        raise RuntimeError("Unreal glTF exporter returned failure")
    if not os.path.isfile(OUTPUT_PATH):
        raise RuntimeError(f"Exporter did not create output: {OUTPUT_PATH}")

    unreal.log(
        f"LAEVAT_EXPORT_SUCCESS path={OUTPUT_PATH} "
